...
router.post('/signin', function(req, res){
    ...
    req.on('end', function () {
        ...
        var jsonObj = JSON.parse(body),
            userLogin = jsonObj.login,
            userPassword = jsonObj.password;
            ...
            User.find({name: userLogin}, function (err, data) { // insert user login and password
                ...
                if (Object.keys(data).length != 0) {
                    ...
                    if ((data[0].name != userLogin) && (userPassword != decryptPass)) {
                        res.send({answer: 'login or password not right'});
                    } else {
                    ...
                    res.send({answer: 'user not exist'});
                }

            });
        }else{
            res.send({answer: 'empty fields'});
        }
    });
});
